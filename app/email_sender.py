...
class EmailSender:
	def __init__(self):
		...
		self.use_mailhog = self.config['email'].get('use_mailhog', 'False').lower() in ['true', '1', 't', 'y', 'yes']
	...
	def send_email_after_6_months(self, information):
		try:
			...
			text = self.format_email_text(self.email_six_text)
			subject = self.email_six_subject
			msg = MIMEMultipart('related')
			msg['From'] = sender_email
			msg['To'] = receiver_email
			msg['Subject'] = subject
	...
	def send_email_after_more_than_6_months(self, information):
		try:
			logo_akema = resource_path("./asset/logo.png")
			logo_GT = resource_path("./asset/logoGT.jpg")
